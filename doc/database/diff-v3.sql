...
ADD INDEX `fk_lancamento_cobranca1_idx` (`id_fatura` ASC) VISIBLE,
DROP INDEX `fk_lancamento_fatura1_idx` ;
;

DROP TABLE IF EXISTS `housecare`.`fatura` ;

ALTER TABLE `housecare`.`cobranca` 
ADD COLUMN `tipo` VARCHAR(2) NULL DEFAULT NULL AFTER `pagamento`, RENAME TO  `housecare`.`fatura` ;
...
ADD CONSTRAINT `fk_lancamento_cliente1`
  ...
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
ADD CONSTRAINT `fk_lancamento_cobranca1`
